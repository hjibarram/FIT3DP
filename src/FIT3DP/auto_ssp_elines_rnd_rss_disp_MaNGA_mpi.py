...
if rank == 0:
    ...
    time1=ssp.get_time()
    if len(sys.argv) < 7:
        print "USE: auto_ssp_elines_rnd_rss_disp_MaNGA_mpi.py SPEC1.RSS.fits,ERROR.RSS.fits SSP_SFH.fits,SSP_KIN.fits OUTFILE MASK_LIST CONFIG_FILE PLOT [min max] [wmin wmax] [redshift_elines_to_mask] [input_redshift delta_redshift min_redshift max_redshift] [input_sigma delta_sigma min_sigma max_sigma] [input_Av delta_Av min_Av max_Av]"
        print "CONFIG_FILE:"
...
#
    ...
    if input_e_rss != "none":
        ...
        statst=my.stats(pdl_e_flux)
        pdl_e_flux[np.where(pdl_e_flux <= 0)[0]]=3*statst[2]
        pdl_e_flux[np.where(pdl_e_flux >= 3*statst[2])[0]]=3*statst[2]
    else:
    ...
    if med_flux > CUT_MEDIAN_FLUX and med_flux > ABS_MIN:
        ...
        if d_redshift > 0:
            ...
            while RED < max_redshift:
                ssp_dat1=ssp.fit_ssp_lin_no_zero_no_cont(RED,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot)
                chi_now=ssp_dat1[0]
            ...
            while RED < max_redshift:
                ssp_dat2=ssp.fit_ssp_lin_no_zero_no_cont(RED,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot)
                chi_now=ssp_dat2[0]
                chi_r.extend([chi_now])
                red_r.extend([RED])
        ...
        if d_sigma > 0:
            ...
            while SIGMA < max_sigma: 
                ssp_dat3=ssp.fit_ssp_lin_no_zero(redshift,SIGMA,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot)
                chi_now=ssp_dat3[0]
            ...
            sigma_s=[]
            while SIGMA < max_sigma:
                ssp_dat4=ssp.fit_ssp_lin_no_zero(redshift,SIGMA,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot)
                chi_now=ssp_dat4[0]
                chi_s.extend([chi_now])
                sigma_s.extend([SIGMA])
                if nr == 0:
        ...
        if d_Av_IN > 0:
            ...
            while Av_NOW < max_Av:
                for i in range(0, nf):
                    Av[i]=Av_NOW 
                ssp_dat5=ssp.fit_ssp_lin_no_zero(redshift,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked_Av,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot)
                chi_now=ssp_dat5[0]
        ...
        if e_Av < 0.0001:
            e_Av=d_Av_IN*0.5
        print "AV = "+str(Av_F)+" +- "+str(e_Av)+" at processor "+str(rank)
        for i in range(0, nf):
            Av[i]=Av_F
        fit_redshift=0
        ...
        [min_chi_sq,pdl_age_mod,pdl_met_mod,pdl_ml,pdl_Av,coeffs,coeffs_N,coeffs_NM,pdl_model_spec_min,pdl_res]=ssp.fit_ssp_lin_no_zero(redshift,sigma,Av,crval1,cdelt1,crpix1,nf,n_c,pdl_flux_c_ini,hdr,wave_unc,masked_Av,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot)
        #
        ...
        while MIN_CHISQ > MIN_DELTA_CHISQ and NITER < MAX_NITER:
            ...
            if med_flux > CUT_MEDIAN_FLUX:
                ...
                if ns > 0:
                    ...
                    [min_chi_sq,pdl_age_mod,pdl_met_mod,pdl_ml,pdl_Av,coeffs,coeffs_N,coeffs_NM,pdl_mod_SSP,pdl_res_SSP,coeffs_N_input,e_coeffs_N_input]=ssp.fit_ssp_lin_MC(redshift,sigma_MaNGA,Av,crval1,cdelt1,crpix1,nf,n_c,pdl_flux_c_ini,hdr,wave_unc,masked,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,MIN_CHISQ,plot)
