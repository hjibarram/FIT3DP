...
time1=ssp.get_time()
if len(sys.argv) < 7:
    print "USE: auto_ssp_elines_rnd_rss_disp_MaNGA.py SPEC1.RSS.fits,ERROR.RSS.fits SSP_SFH.fits,SSP_KIN.fits OUTFILE MASK_LIST CONFIG_FILE PLOT [min max] [wmin wmax] [redshift_elines_to_mask] [input_redshift delta_redshift min_redshift max_redshift] [input_sigma delta_sigma min_sigma max_sigma] [input_Av delta_Av min_Av max_Av]"
    print "CONFIG_FILE:"
...
pdl_wave_org=crval+cdelt*np.arange(0, nx)
...
#
    ...
    if med_flux > CUT_MEDIAN_FLUX and med_flux > ABS_MIN:
        ...
        print "Signal-to-Noise = "+str(SN)+" ("+str(stats_mod)+"/"+str(stats_res)+")"
        old=1
        if old == 1 and SN > 10:
