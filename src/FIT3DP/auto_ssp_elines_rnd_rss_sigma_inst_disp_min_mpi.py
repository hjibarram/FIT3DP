...
data=filter(None,data)
Av_IN=float_(data[0])
d_Av_IN=float_(data[1])
min_Av=float_(data[2])
max_Av=float_(data[3])
if len(sys.argv) == 24:
    min=float_(sys.argv[7])
    max=float_(sys.argv[8])
    min_wave=sys.argv[9]
...
if elines_mask == "none":
    ...
    for line in f:
        ...
        if data[0] != "#":
            ...
            start_mask_e.extend([w_eline[nline]*(1+input_redshift)-4.0])
            end_mask_e.extend([w_eline[nline]*(1+input_redshift)+4.0])
            nmask_e=nmask_e+1
            nline=nline+1
    f.close()
#
...
for jy in range(i_in, i_fi):
    ...
    if jy == i_in:
        ...
        if sigma > 0:
            sigma_seq.extend([sigma])
            k_seq=k_seq+1
        n_seq_last=int(0.2*jy)
        if n_seq_last < 10:
            n_seq_last=10
        if k_seq > n_seq_last:
            sigma_seq_last=[]
            for i_now in range(0, n_seq_last):
                sigma_seq_last.extend([sigma_seq[k_seq-1-i_now]])
            sigma=np.median(sigma_seq_last)
        print "SIGMA_SEQ = "+str(sigma)+" , "+str(k_seq)
        min_sigma=sigma-INPUT_D_SIGMA
        max_sigma=sigma+INPUT_D_SIGMA
        d_sigma=0.25*INPUT_D_SIGMA
...
#
    ...
    if med_flux > CUT_MEDIAN_FLUX and med_flux > ABS_MIN:
        ssp_dat0=ssp1.fit_ssp_lin_no_zero_no_cont(redshift,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked_Av,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot,sigma_inst)
        ...
        if d_redshift > 0:
            ...
            while RED < max_redshift:
                ssp_dat1=ssp1.fit_ssp_lin_no_zero_no_cont(RED,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot,sigma_inst)
                chi_now=ssp_dat1[0]
            ...
            while RED < max_redshift:
                ssp_dat2=ssp1.fit_ssp_lin_no_zero_no_cont(RED,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot,sigma_inst)
                chi_now=ssp_dat2[0]
                chi_r.extend([chi_now])
                red_r.extend([RED])
            ...
            fit_redshift=0
            e_redshift=0
        print "REDSHIFT = "+str(redshift)+" +- "+str(e_redshift)+" at processor "+str(rank)
        REDSHIFT=redshift
        # sigma DETERMINATION 
        K=0
        nr=0
        ...
        if d_sigma > 0:
            ...
            while SIGMA < max_sigma: 
                ssp_dat3=ssp1.fit_ssp_lin_no_zero_no_cont(redshift,SIGMA,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot,sigma_inst)
                chi_now=ssp_dat3[0]
                ...
                SIGMA=SIGMA+d_sigma*(ran.rand())-0.25*d_sigma*(ran.rand())
                d_sigma=d_sigma+0.2*d_sigma*(ran.rand())  
            e_sigma=d_sigma
            d_sigma_ini=INPUT_D_SIGMA
            SIGMA=sigma-1.5*d_sigma_ini
            if SIGMA < 0:
                SIGMA=10
            #print "VAL_S = "+str(sigma)+" +- "+str(e_sigma)  
            max_sigma=sigma+1.5*d_sigma_ini
            d_sigma=0.33*d_sigma_ini
            #
            #TWO
            #
            nr=0
            ...
            sigma_s=[]
            while SIGMA < max_sigma:
                ssp_dat4=ssp1.fit_ssp_lin_no_zero_no_cont(redshift,SIGMA,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot,sigma_inst)
                chi_now=ssp_dat4[0]
                chi_s.extend([chi_now])
                sigma_s.extend([SIGMA])
                #if rank == 2:
                #    print SIGMA,chi_now,jy
                if nr == 0:
                    sigma=sigma_s[nr]
                    min_chi_sq=chi_now
                ...
                nr=nr+1
                SIGMA=SIGMA+d_sigma
                d_sigma=d_sigma+0.05*d_sigma*ran.rand()
            if sigma_s[nr-1] != sigma:
                slope=(chi_s[nr-1]-min_chi_sq)/(sigma_s[nr-1]-sigma)
                if slope != 0:
                    e_sigma=sigma/slope/300000.0/10.0
                else:
                    e_sigma=d_sigma
            else:
                e_sigma=d_sigma
        ...
        sigma=abs(sigma)
        e_sigma=abs(e_sigma)
        pdl_sigma=np.array(sigma_s)
        pdl_chi=np.array(chi_s)
        sigma_sum=np.sum(pdl_sigma/(pdl_chi)**4.0)
        chi_sum=np.sum(1/(pdl_chi)**4.0)
        sigma_sum=sigma_sum/chi_sum
        stats_sigma=my.stats(pdl_sigma/(pdl_chi)**4.0)    
        stats_chi=my.stats(1/(pdl_chi)**4.0)
        e_sigma=0.33*(stats_sigma[1]/stats_chi[0])
        if sigma_sum > INPUT_MIN_SIGMA and sigma_sum < INPUT_MAX_SIGMA:
            sigma=sigma_sum
        print "SIGMA = "+str(sigma)+"+-"+str(e_sigma)+" at processor "+str(rank)
        # Av DETERMINATION
        K=0
        nr=0
        chi_Av=[]
        ...
        if d_Av_IN > 0:
            ...
            while Av_NOW < max_Av:
                for i in range(0, nf):
                    Av[i]=Av_NOW 
                ssp_dat5=ssp1.fit_ssp_lin_no_zero(redshift,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked_Av,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot,sigma_inst)
                chi_now=ssp_dat5[0]
        ...
        if Av_F == 0:
            Av_F=0.00001
        print "AV = "+str(Av_F)+" +- "+str(e_Av)+" at processor "+str(rank)#,d_Av_IN
        #sys.exit()
        for i in range(0, nf):
            Av[i]=Av_F
        fit_redshift=0
        ...
        [min_chi_sq,pdl_age_mod,pdl_met_mod,pdl_ml,pdl_Av,coeffs,coeffs_N,coeffs_NM,pdl_model_spec_min,pdl_res]=ssp1.fit_ssp_lin_no_zero(redshift,sigma,Av,crval1,cdelt1,crpix1,nf,n_c,pdl_flux_c_ini,hdr,wave_unc,masked_Av,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot,sigma_inst)
        #
        ...
        print "Signal-to-Noise = "+str(SN)+" at processor "+str(rank)
        old=1       
        if old == 1 and SN > 10:
