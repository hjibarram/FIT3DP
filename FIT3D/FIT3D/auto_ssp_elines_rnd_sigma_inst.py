...
ssp_dat=ssp1.fit_ssp_lin_no_zero(redshift,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked_Av,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot,sigma_inst)
...
# REDSHIFT DETERMINATION
    ...
    if d_redshift > 0:
        ...
        while RED < max_redshift:
            ssp_dat1=ssp1.fit_ssp_lin_no_zero_no_cont(RED,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc_kin,flux_masked2,n_mc,chi_sq_min_now,min_chi_sq,my_plot,sigma_inst)
            chi_now=ssp_dat1[0]
            chi_r.extend([chi_now])
            red_r.extend([RED])
#            print RED,chi_now,d_redshift
            #print redshift,sigma,min_chi_sq
            if nr > 1  and chi_r[nr-1] < min_chi_sq and chi_r[nr-1] > 0:
                redshift=red_r[nr-1]    
                min_chi_sq=chi_r[nr-1]    
                K=nr-1
                #print redshift,sigma,min_chi_sq,"TEST_1"
            nr=nr+1
            RED=RED+d_redshift
        #
        # TWO
        ...
        while RED < max_redshift:
            ssp_dat2=ssp1.fit_ssp_lin_no_zero_no_cont(RED,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_masked2,n_mc,chi_sq_min_now,min_chi_sq,my_plot,sigma_inst)
            chi_now=ssp_dat2[0]
            chi_r.extend([chi_now])
            red_r.extend([RED])
            #print redshift,sigma,min_chi_sq
            if nr > 1 and chi_r[nr-1] < chi_r[nr-2] and chi_r[nr-1] < chi_r[nr] and chi_r[nr-1] <= min_chi_sq:
                ...
                min_chi_sq=chi_r[nr-1]    
                K=nr-1   
                #print redshift,sigma,min_chi_sq,"TEST_2"
            nr=nr+1
            a_rnd=ran.rand(2)
    ...
    if d_sigma > 0:
        ...
        while SIGMA < max_sigma: 
            #chi_now,pdl_age_mod,pdl_met_mod,pdl_ml,pdl_Av,coeffs
            ssp_dat3=ssp1.fit_ssp_lin_no_zero(redshift,SIGMA,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_masked2,n_mc,chi_sq_min_now,min_chi_sq,my_plot,sigma_inst)
            ...
            if chi_s[nr-1] < min_chi_sq:
                ...
                K=nr
                #print redshift,sigma,sigma_inst,Av,min_chi_sq,"TEST_3"
            nr=nr+1
            SIGMA=SIGMA+d_sigma*ran.rand()-0.25*d_sigma*ran.rand()  
            d_sigma=d_sigma+0.25*d_sigma*ran.rand()
        sigma_broad=sigma
        ...
        while SIGMA < max_sigma:
            #chi_now,pdl_age_mod,pdl_met_mod,pdl_ml,pdl_Av,coeffs
            ssp_dat4=ssp1.fit_ssp_lin_no_zero(redshift,SIGMA,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked2,e_flux_unc,flux_masked2,n_mc,chi_sq_min_now,min_chi_sq,my_plot,sigma_inst)
            ...
            if nr > 0:
                if chi_s[nr-1] < min_chi_sq:
                    ...
                    min_chi_sq=chi_s[nr-1]
                    K=nr
                    #print redshift,sigma,sigma_inst,Av,min_chi_sq,"TEST_4"
    ...
    chi_sum=np.sum(1.0/pdl_chi**4.0)
    ...
    if sigma_sum > INPUT_MIN_SIGMA and sigma_sum < INPUT_MAX_SIGMA :
        e_sigma=np.sqrt(e_sigma**2.0+(sigma-sigma_sum)**2.0)
        sigma=sigma_sum
    print "SIGMA = "+str(sigma)+"+-"+str(e_sigma)+" "+str(sigma_sum)
else:
#
# Below the cut!
#
...
if d_Av_IN > 0:
    ...
    while Av_NOW < max_Av:
        ...
        #
        ssp_dat5=ssp1.fit_ssp_lin_no_zero(redshift,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked_Av,e_flux_unc,flux_masked,n_mc,chi_sq_min_now,min_chi_sq,my_plot,sigma_inst)
        chi_now=ssp_dat5[0]
...
[min_chi_sq,pdl_age_mod,pdl_met_mod,pdl_ml,pdl_Av,coeffs,coeffs_N,coeffs_NM,pdl_model_spec_min,pdl_res]=ssp1.fit_ssp_lin_no_zero(redshift,sigma,Av,crval2,cdelt2,crpix2,nf2,n_c2,pdl_flux_c_ini2,hdr2,wave_unc,masked_Av,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,min_chi_sq,plot,sigma_inst)
#
...
###############################
        ...
        if med_flux > CUT_MEDIAN_FLUX:
            n_mc=20
            [min_chi_sq,pdl_age_mod,pdl_met_mod,pdl_ml,pdl_Av,coeffs,coeffs_N,coeffs_NM,pdl_mod_SSP,pdl_res_SSP,coeffs_N_input,e_coeffs_N_input]=ssp1.fit_ssp_lin_MC(redshift,sigma,Av,crval,cdelt,crpix,nf,n_c,pdl_flux_c_ini,hdr,wave_unc,masked,e_flux_unc,flux_unc,n_mc,chi_sq_min_now,MIN_CHISQ,plot,sigma_inst)
            sigma_mean=np.sqrt(sigma_inst**2.0+(5000.0*(sigma/300000.0))**2.0)
